...
class FirebaseAuthProvider(config: FirebaseConfig) : AuthenticationProvider(config) {
    ...
    private val realm = config.name ?: throw IllegalArgumentException("Firebase realm must be specified")

    override suspend fun onAuthenticate(context: AuthenticationContext) {
        println("token: ${context.call}")
        ...
        if (token == null) {
            context.challenge(
                ...
                call.respond(UnauthorizedResponse(HttpAuthHeader.bearerAuthChallenge(realm = realm)))
            }
            return
        }
